"""Tests for aura.db.extraction_channel_config: the per-channel on/off switch
for automatic fact extraction (Phase 3a).

Deliberately mirrors tests/test_channel_config.py case for case: this module
is a sibling of aura.db.proactive_channel_config with the same shape and the
same invariants (see schema.sql's extraction_channel_config comment for why
it is a separate table rather than a second column), so it earns the same
depth of evidence, including the same concurrency test -- see
TestConcurrency's docstring for why that race class applies here too.
...
from aura.db.extraction_channel_config import is_extraction_enabled, set_extraction_enabled
...
class TestDefaultOff:
    async def test_an_unconfigured_channel_is_disabled(self, conn: aiosqlite.Connection) -> None:
        # The load-bearing default: extraction is opt-in per channel.
        assert await is_extraction_enabled(conn, channel_id=999) is False

    async def test_an_empty_database_disables_every_channel(
        self, conn: aiosqlite.Connection
    ) -> None:
        for channel_id in (1, 2, 3, 10**18):
            assert await is_extraction_enabled(conn, channel_id=channel_id) is False
...
class TestSetAndGet:
    async def test_enabling_a_channel_turns_it_on(self, conn: aiosqlite.Connection) -> None:
        ...
        assert await is_extraction_enabled(conn, channel_id=5) is True
    ...
    async def test_disabling_a_channel_turns_it_off(self, conn: aiosqlite.Connection) -> None:
        ...
        assert await is_extraction_enabled(conn, channel_id=5) is False
    ...
    async def test_toggling_repeatedly_leaves_exactly_one_row(
        ...
        async with conn.execute(
            "SELECT COUNT(*) FROM extraction_channel_config WHERE channel_id = 5"
        ) as cursor:
            assert await cursor.fetchone() == (1,)
        assert await is_extraction_enabled(conn, channel_id=5) is True  # the last write wins
    ...
    async def test_the_editor_and_time_are_recorded_for_audit(
        ...
        async with conn.execute(
            "SELECT updated_by_id, updated_at FROM extraction_channel_config WHERE channel_id = 5"
        ) as cursor:
    ...
    async def test_a_later_edit_updates_the_editor(self, conn: aiosqlite.Connection) -> None:
        ...
        async with conn.execute(
            "SELECT updated_by_id FROM extraction_channel_config WHERE channel_id = 5"
        ) as cursor:
...
class TestChannelIsolation:
    async def test_enabling_one_channel_does_not_enable_another(
        ...
        assert await is_extraction_enabled(conn, channel_id=5) is True
        assert await is_extraction_enabled(conn, channel_id=6) is False
...
class TestIndependenceFromProactiveConfig:
    """The whole point of a separate table: neither gate can see the other's state."""

    async def test_enabling_extraction_does_not_enable_proactive_relief(
        self, conn: aiosqlite.Connection
    ) -> None:
...
class TestConcurrency:
    async def test_concurrent_writes_to_one_channel_leave_one_consistent_row(
        ...
        # moderator racing themselves) must not corrupt the single row. This is
        # a rare, human-triggered admin action -- as it is for
        # proactive_channel_config -- so the property under test is data
        # integrity, not throughput: whichever write lands last must produce a
        # single valid row, never a duplicate or a torn one.
        ...
        async with conn.execute(
            ...
            assert await cursor.fetchone() == (1,)
        assert isinstance(await is_extraction_enabled(conn, channel_id=5), bool)
