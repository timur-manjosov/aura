...
**The structural fix this still doesn't solve.** The knowledge model has had a `status: active/superseded` field with successor chaining since Phase 1b, but the mod-facing command to actually mark a fact as superseded was deliberately left out of Phase 1c and still does not exist. Until it does, an old fact and the fact that replaced it both sit in the database as "active" at once, and Aura has no structural way to know which one currently holds — only the per-call inference above, which is approximate by nature. **Building that command is the recommended next small phase after this one.** It was not built here; this phase stays scoped to recalibration and prompt sharpening.
